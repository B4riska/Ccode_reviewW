...
def generate_search_url(game_title):
    """
    Генерация поисковой ссылки на игру в Steam.
    """
    search_query = quote(f"{game_title}")  # Кодируем запрос для URL
    ...
    with dbase.connect() as con:
        ...
        for item in range(1, 15, 1):
            ...
            for counter in range(min(len(game_titles), len(game_date_of_release), len(game_prices), len(game_reviews),
                ...
                price = game_prices[counter].text.strip()
                if price.lower() == 'free':
                    price = '0 руб'  # Заменяем "Free" на "0 руб"

                # Генерация поисковой ссылки
                search_url = generate_search_url(game_titles[counter].text.strip())

                cur.execute('''
                    INSERT INTO games (titles, release_date, price, review, description, developer, publisher, min_req, url)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (game_titles[counter].text.strip(), game_date_of_release[counter].text.strip(),
                      price, text_content, games_descriptions[counter].strip(), games_developers[counter].strip(),
                      games_publishers[counter].strip(), min_req_str_list[counter], search_url))
            con.commit()
