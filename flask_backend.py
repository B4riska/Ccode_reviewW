...
def filter_games():
    ...
    if free_games:
        cursor.execute('SELECT * FROM games WHERE price = "0 руб"')
    else:
        if min_price and max_price:
            cursor.execute('SELECT * FROM games WHERE CAST(price AS INTEGER) BETWEEN ? AND ? AND price != "0 руб"',
                           (min_price, max_price))
        elif min_price:
            cursor.execute('SELECT * FROM games WHERE CAST(price AS INTEGER) >= ? AND price != "0 руб"',
                           (min_price,))
        elif max_price:
            cursor.execute('SELECT * FROM games WHERE CAST(price AS INTEGER) <= ? AND price != "0 руб"',
                           (max_price,))
